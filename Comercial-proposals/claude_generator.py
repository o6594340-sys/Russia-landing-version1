...
{{
  "days": [
    {{
      ...
      "morning": "09:00 — трансфер из аэропорта Нарита (~75 мин)\n11:00 — заселение в отель, свободное время\n13:00 — обед: Tonkatsu Maisen, Омотэсандо",
      "afternoon": "14:30 — Сибуя: перекрёсток, Хачико, магазины (~2 ч)\n17:00 — Мейдзи-дзингу: главные ворота Тории, лесная дорожка (~1 ч)",
      "evening": "19:00 — ужин: тэппаньяки Имасэ, Синдзюку\n21:30 — вид на ночной Токио с Tokyo Metropolitan Government Building"
    }}
  ]
}}

ФОРМАТ ПОЛЕЙ morning / afternoon / evening:
Каждое поле — несколько строк, разделённых \n. Каждая строка: «ЧЧ:ММ — Активность или место (краткая деталь)»
...
Примеры качественных строк:
  «09:00 — Цукидзи: внешний рынок, тунец, тамагояки (~1 ч)»
  «11:00 — Мастер-класс каллиграфии в Асакуса (~1.5 ч)»
  «13:30 — Обед: рамен Фуурин Каидо, Синдзюку — очередь 15 мин, стоит»
  «16:00 — TeamLab Planets, Тоёсу (~1.5 ч): цифровые инсталляции без касания»
  «19:30 — Ужин якинику: Торадзи, Роппонги — японская мраморная говядина»
...
{{
  ...
  "hotel_description": "2–3 предложения про {hotel_name if hotel_name else 'отель'}. Расположение, вид из номеров или из ресторана, одна конкретная деталь сервиса или архитектуры которая запомнится. Не общие слова про «уровень 5*».",

...
3.  Программа (обзор)           ← 5 колонок по дням с таймингом, все названия на русском
4.  Концепция программы         ← bridge_bullets[5] от Claude API
5.  Гастрономия (вводный)       ← gastronomy_title / subtitle от Claude API
─── День 1 (сепаратор) ──────────
6.  Прилёт / трансфер
─── День 2 ──────────────────────
7.  Народный парк               ← 3 фото
8.  Музей истории Шанхая        ← 1 фото
9.  Обед — ROOF 325             ← ресторан
10. Набережная Бунд             ← 3 фото
11. Речной круиз по Хуанпу      ← 2 фото
12. Ужин — Lost Heaven on Bund  ← ресторан
─── День 3 (Сучжоу) ────────────
13. Музей Сучжоу                ← 3 фото
14. Сад Мастера сетей           ← 1 фото
15. Обед — Restaurant Mulan     ← ресторан
16. Мастер-класс: гравировка    ← 1 фото
17. Ужин — MeiLongZhen          ← ресторан
─── День 4 ──────────────────────
18. Французская концессия       ← 3 фото
19. Синьтяньди                  ← 3 фото
...
### Слайд программы (обзор, слайд 3)

Функция `slide_program_overview()` — 5 колонок по дням:
- Заголовок колонки: ДЕНЬ N / дата / город
- Пункты расписания с временем (09:30, 13:00 и т.д.)
- Активности — белым, рестораны — зелёным (Обед / Ужин)
- Названия достопримечательностей берутся из поля `name_ru` (русский) в данных proposal
- Сортировка по времени автоматическая

### Структура данных активности в proposal

```python
{
  "name": "The Bund",          # оригинальное название — для поиска фото в photo_bank
  "name_ru": "Набережная Бунд",# русское название — для слайда программы
  "time": "14:30",             # время — для слайда программы
  "note": "..."                # заметки для Claude API (генерирует описание)
}
```
...
Важно: в вызовах `slide_activity_*` и `slide_restaurant` передаётся `photo_name` с оригинальным английским названием объекта — чтобы slug совпадал с папкой в photo_bank. Параметр `title`/`name` используется только для отображения на слайде.

### Папки с фото-банком (photo_bank/)

Структура: `photo_bank/{object-slug}/{slot-filename}.jpg`

Slug строится из оригинального английского названия объекта (строчные буквы, пробелы → дефисы).
...
### Принципы текстогенерации (system prompt в text_generator.py)

Промпт прошёл ревью копирайтера и MICE-продавца. Ключевые правила:

**Содержание:**
- Один неочевидный факт на слайд — цифра, год, имя, история — чтобы агент мог пересказать клиенту
- Субъективные оценки заменяются наблюдаемыми деталями (не «вкус яркий», а что стоит на столе)
- Последнее предложение — самое сильное (образ или деталь, не логистика и не «это хорошее место»)
- Мастер-классы: обязательно описать предмет, который человек увозит домой
- Отели: никаких извинений за категорию, только конкретные преимущества

**Язык:**
- Русский без калек с английского — ни в лексике, ни в пунктуации
- Обращение на «вы», тон спокойный и немного вдохновляющий
- Запрет: «уникальный», «незабываемый», «богатая история», «жемчужина», «атмосфера»
- Запрет на академические кальки: «биологически разнообразный», «культурно значимый»

**Пунктуация:**
- Тире только когда между подлежащим и сказуемым подразумевается «это» («Сучжоу — город каналов»)
- Во всех остальных случаях: запятая, точка или ничего
- Перед «и» запятая только между двумя самостоятельными предложениями
